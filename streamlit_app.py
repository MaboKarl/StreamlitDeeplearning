...
class GestureDetector(VideoProcessorBase):
    def __init__(self):
        # Load model reference once inside processor — avoids closure issues
    ...
    def recv(self, frame):
        ...
        if len(boxes) > 0:
            ...
            cv2.putText(annotated, f"Gesture: {label} ({conf:.2f})",
                        (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
            cv2.putText(annotated, f"Action: {action}",
                        (10, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 0), 1)
        else:
            cv2.putText(annotated, "No gesture",
...
webrtc_streamer(
    ...
    # ✅ KEY FIX: prevents Streamlit from re-rendering the component on each frame
